...
Interface into VOXL via self-hosted web browser
...
```bash
~/git/voxl-portal$ voxl-docker -i voxl-cross
voxl-cross:~$
```
...
The build script in this template will build 32-bit and 64-bit using different cross compilers for each hardware platform.
...
```bash
(outside of docker)
voxl-portal$ ./deploy_to_voxl.sh ssh 192.168.1.123
```

## Debug Output

To view the debug output of this service using ADB

```
adb shell journalctl -f -u voxl-portal
```