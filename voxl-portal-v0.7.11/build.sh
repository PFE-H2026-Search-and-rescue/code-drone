...
case "$1" in
	apq8096)
		...
		cd build64
        BUILD_QRB5165="OFF"
		cmake -DCMAKE_TOOLCHAIN_FILE=${TOOLCHAIN_APQ8096_64} -DBUILD_QRB5165=${BUILD_QRB5165} ${EXTRA_OPTS} -DCMAKE_CXX_FLAGS="${CMAKE_CXX_FLAGS}" ../
		make -j$(nproc)
		cd ../
		;;
	qrb5165)
		mkdir -p build64
		cd build64
		cmake -DCMAKE_TOOLCHAIN_FILE=${TOOLCHAIN_QRB5165} -DBUILD_QRB5165=${BUILD_QRB5165} ${EXTRA_OPTS} ../
		make -j$(nproc)
		cd ../
		;;
